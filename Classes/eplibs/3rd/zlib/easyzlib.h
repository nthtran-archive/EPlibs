 ...
 * Usages
 *
 char *xml = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>"
 "<note>"
 "<to>Tove</to>"
 "<from>Jani</from>"
 ...
 int xmlSize = (int)strlen(xml);
 ezbuffer b1(xmlSize),b2,b3;
 memcpy(b1.pBuf,xml,xmlSize);
 ezcompress(b2, b1);
 ezuncompress(b3, b2);
...
int ezcompress( ezbuffer& bufDest, const ezbuffer& bufSrc )
{
	...
	{
		bufDest.Alloc( (int)bufDest.nLen );
